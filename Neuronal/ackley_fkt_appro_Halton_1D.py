...
y=(-20 * exp(-0.2 * sqrt(0.5 * (x**2)))-exp(0.5 * (cos(2 * pi * x))) + np.e + 20)
...
model = build_model()
model.summary()
mc_best = tf.keras.callbacks.ModelCheckpoint("ackley_fkt_appro_Halton_1Dto1D" + '/best_model', monitor='val_loss', mode='min',
...
model= tf.keras.models.load_model("ackley_fkt_appro_Halton_1Dto1D/best_model")
