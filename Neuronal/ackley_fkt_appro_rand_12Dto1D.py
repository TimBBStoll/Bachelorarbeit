...
import numpy as np
from random_korrekt import rand_12d
...
y=(-20 * exp(-0.2 * sqrt(0.5 * (z)))-exp(0.5 * (cos(2 * pi * a)+cos(2 * pi * b)+cos(2 * pi * c)+cos(2 * pi * d)+cos(2 * pi * e)+cos(2 * pi * f)+cos(2 * pi * g)+cos(2 * pi *h)+cos(2 * pi * i)+cos(2 * pi * j)+cos(2 * pi * k)+cos(2 * pi * l))) + np.e + 20)
...
model = build_model()
model.summary()
mc_best = tf.keras.callbacks.ModelCheckpoint("ackley_fkt_appro_rand_12Dto1D" + '/best_model', monitor='val_loss', mode='min',
...
model= tf.keras.models.load_model("ackley_fkt_appro_rand_12Dto1D/best_model")
