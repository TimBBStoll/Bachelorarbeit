...
from Halton_korrekt import Halton_4D
...
y=30*(a)**(4)-60*(a)**(3)+35*(a)**(2)-4.5*(a)+30*(b)**(4)-60*(b)**(3)+35*(b)**(2)-4.5*(b)+30*(c)**(4)-60*(c)**(3)+35*(c)**(2)-4.5*(c)+30*(d)**(4)-60*(d)**(3)+35*(d)**(2)-4.5*(d)
...
model = build_model()
model.summary()
mc_best = tf.keras.callbacks.ModelCheckpoint("poly_fkt_appro_Halton_4Dto1D" + '/best_model', monitor='val_loss', mode='min',
...
model= tf.keras.models.load_model("poly_fkt_appro_Halton_4Dto1D/best_model")
